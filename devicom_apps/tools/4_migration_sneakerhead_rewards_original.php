...
//}
echo "GETTING ORDERS\n";
$query = "SELECT `increment_id` FROM `sales_flat_order` WHERE `increment_id` >= '600000000' AND `customer_id` IS NOT NULL";
$results = $writeConnection->query($query);
foreach ($results as $result) {
	...
	foreach ($getPointsResults as $getPointsResult) {  
	    //Update POINTS WAITING
	    $newValue = $getPointsResult['points_waiting'] + $pointsCurrent;
	    $query = "UPDATE `rewardpoints_flat_account` SET `points_waiting` = " . $newValue . " WHERE `user_id` = " . $customerId . " AND `store_id` = 21";
	    $writeConnection->query($query);
...
foreach ($results as $result) {
	...
	//Get existing points
	$query = "SELECT * FROM `rewardpoints_flat_account` WHERE `user_id` = " . $customerResult['entity_id'] . " AND `store_id` = 21";
	$getPointsResults = $writeConnection->query($query);
	foreach ($getPointsResults as $getPointsResult) {  
	    //Update POINTS COLLECTED and POINTS CURRENT
	    $newValue = $getPointsResult['points_collected'] + $result['points_current'];
	    $newValue2 = $getPointsResult['points_current'] + $result['points_current'];
	    $query = "UPDATE `rewardpoints_flat_account` SET `points_collected` = " . $newValue . ",`points_current` = " . $newValue2 . " WHERE `user_id` = " . $customerResult['entity_id'] . " AND `store_id` = 21";
	    $writeConnection->query($query);
	    fwrite($transactionLogHandle, "    ->UPDATED   : FLAT POINTS FOR Customer ID  : " . $customerResult['entity_id'] . "\n");
	}
...
foreach ($results as $result) {
	...
	$getPointsResults = $writeConnection->query($query);
	foreach ($getPointsResults as $getPointsResult) {  
	    //Update POINTS USED and POINTS CURRENT
	    $newValue = $getPointsResult['points_used'] + $result['points_spent'];
	    $newValue2 = $getPointsResult['points_current'] - $result['points_spent'];
	    $query = "UPDATE `rewardpoints_flat_account` SET `points_used` = " . $newValue . ",`points_current` = " . $newValue2 . " WHERE `user_id` = " . $customerResult['entity_id'] . " AND `store_id` = 21";
	    $writeConnection->query($query);
	    fwrite($transactionLogHandle, "    ->UPDATED   : FLAT POINTS FOR Customer ID  : " . $customerResult['entity_id'] . "\n");
	}
	fwrite($transactionLogHandle, "  ->DONE        : " . $result['email'] . "\n");

...
if (!rtrim(basename(shell_exec('ls /chroot/home/rxkicksc/rxkicks.com/html/devicom_apps/tools/logs/*.lock | head --lines 1')))&& $filename = rtrim(basename(shell_exec('ls -t -r /chroot/home/rxkicksc/rxkicks.com/html/devicom_apps/tools/xml_files/full_order_status_update.xml | head --lines 1')))) {
    ...
    $lastOrderNumberForProcessing = '600402720';//191298 through 600402720 (complete), 600402721 through 600408224 (processing)
    ...
    try {
	    ...
	    fwrite($transactionLogHandle, $orderNumber . "\n");

            //Add only orders that are less than or equal to last order number
            if ($orderNumber <= $lastOrderNumberForProcessing || $orderNumber >= '700000000') {
                continue;
            }
        ...
        if (isset($elements['Orders'])) {
            ...
            foreach ($orderUpdates as $orderUpdate) {
                ...
                if ($startItem <= $itemCounter && $endItem >= $itemCounter) {
                    ...
                    if (!$order->getId()) {
                        fwrite($transactionLogHandle, "    ->ORDER       : ORDER NOT FOUND ->" . $orderUpdate['orderNumber'] . "\n");
                        continue;
                    }
		    ...
		    //Add all order items to devicom_order_items table
		    $query = "SELECT * FROM `devicom_order_items` WHERE `increment_id` = '" . $orderUpdate['orderNumber'] . "'";
			...
			foreach ($order->getAllItems() as $orderItem) {
				...
				if ($orderItem->getSku() == $itemUpdate['sku'] && !array_key_exists($itemUpdate['sku'], $skuArray)) {
				    $query = "INSERT INTO `devicom_order_items` (`increment_id`, `sku`, `quantityOrdered`, `quantityShipped`, `quantityReturned`, `quantityBackordered`) VALUES ('" . $orderUpdate['orderNumber'] . "', '" . $itemUpdate['sku'] . "', " . $itemUpdate['quantityOrdered'] . ", " . $itemUpdate['quantityShipped'] . ", " . $itemUpdate['quantityReturned'] . ", " . $itemUpdate['quantityBackordered'] . ")";
			...
			foreach ($order->getAllItems() as $orderItem) {
			    foreach ($orderUpdate['items'] as $itemUpdate) {
				if ($orderItem->getSku() == $itemUpdate['sku'] && !array_key_exists($itemUpdate['sku'], $skuArray)) {
				    $query = "UPDATE `devicom_order_items` SET `quantityOrdered` = " . $itemUpdate['quantityOrdered'] . ",`quantityShipped` = " . $itemUpdate['quantityShipped'] . ",`quantityReturned` = " . $itemUpdate['quantityReturned'] . ",`quantityBackordered` = " . $itemUpdate['quantityBackordered'] . " WHERE `increment_id` = '" . $orderUpdate['orderNumber'] . "' AND `sku` = '" . $itemUpdate['sku'] . "'";
				    $skuArray[$itemUpdate['sku']] = $itemUpdate['sku'];
				    $writeConnection->query($query);
		    ...
		    //Set status and state
				    ...
				    if ($customerId) {
					$query = "INSERT IGNORE INTO `devicom_reward_validation` (`increment_id`, `status`, `shipped_on`) VALUES ('" . $orderUpdate['orderNumber'] . "', 'backordered', '" . $orderDate . "')";
					$writeConnection->query($query);
				    ...
				    if ($customerId) {
					$query = "INSERT IGNORE INTO `devicom_reward_validation` (`increment_id`, `status`, `shipped_on`) VALUES ('" . $orderUpdate['orderNumber'] . "', 'cancel', '" . $orderDate . "')";
					$writeConnection->query($query);
				...
				if ($orderUpdate['orderNumber'] <= $lastOrderNumberForProcessing) {
				    ...
				    if ($customerId) {
					$query = "INSERT IGNORE INTO `devicom_reward_validation` (`increment_id`, `status`, `shipped_on`) VALUES ('" . $orderUpdate['orderNumber'] . "', 'shipped', '" . $orderUpdate['shipDate'] . "')";
					$writeConnection->query($query);

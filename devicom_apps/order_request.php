...
$salesLogsDirectory = $rootPath . '/devicom_apps/sales/logs/';
$realTime = realTime();
...
//if ($_SERVER['REQUEST_METHOD'] == 'POST' && in_array($ip, $allowed)) {
	...
	// Load Magento core
	//PCN BEGIN
	$mageFilename = $rootPath . '/app/Mage.php';
	//PCN END
...
final class StoneEdge_MagentoImport {
	...
	public function dispatcher($rootPath) {
		try {
	...
	private static function getStore() {
		...
		try {
			if (self::$_debug) {
				echo "Acquiring app handle for store code '$code'...\r\n";
			}
			$app = Mage::app($code);
			self::$_storeId = $app->getStore()->getData('store_id');
			if (self::$_debug) {
				echo "Using store id '" . self::$_storeId . "'.\r\n";
			}
		} catch (Exception $e) {
			...
			//Append error to exception log
			$salesLogsDirectory = $core_dir . '/html/devicom_apps/sales/logs/';
			$handle = fopen($salesLogsDirectory . 'exception_log', 'a');
	...
	private static function getOrderEntityId($db, $sql, DateTime $lastDate, $lastOrder,$storeId=1) {
		...
		$sql = $db->select()
				...
				->where("increment_id = '$lastOrder' OR updated_at >= '{$lastDate->format('Y-m-d H:i:s')}'")  //increment_id = '$lastOrder' OR
// ALERTBOT BEGIN => Exclude orders from sneakerhead.com domain
				->where("coalesce(`customer_email`, '') NOT IN ('alertbot@sneakerhead.com')")
// ALERTBOT END
				->order('entity_id');
		if (self::$_debug) {
			echo 'Executing SQL: ' . $sql->__toString() . "\r\n";
		}
	...
	private static function ordercount() {
		...
		if ($lastOrder) {
			$lastEntityId = self::getOrderEntityId($db, $sql, $lastDate, $lastOrder);
		}
		$sql = $db->select()
	...
	private static function writeCustAddress(DOMElement $ndAddr, Mage_Customer_Model_Address $addr, $email, DOMDocument $xd) {

		$street = explode("\n", $addr->getStreetFull());
		self::xmlAppend("FirstName", $addr->getData('firstname'), $ndAddr, $xd);
		self::xmlAppend("LastName", $addr->getData('lastname'), $ndAddr, $xd);
		if ($email != '') {
			self::xmlAppend("Email", $email, $ndAddr, $xd);
		}
		self::xmlAppend("Company", $addr->getData('company'), $ndAddr, $xd);
//PCN BEGIN
		self::xmlAppend("Phone", preg_replace("/[^0-9\.\+\-\s\(\)]/", "", $addr->getData('telephone')), $ndAddr, $xd);
		self::xmlAppend("Fax", preg_replace("/[^0-9\.\+\-\s\(\)]/", "", $addr->getData('fax')), $ndAddr, $xd);
//PCN END
		if (sizeof($street) > 0) {
			self::xmlAppend("Addr1", $street[0], $ndAddr, $xd);
		}
		if (sizeof($street) > 1) {
			self::xmlAppend("Addr2", $street[1], $ndAddr, $xd);
		}
	...
	private static function writeProduct(DOMElement $ndProd, Mage_Catalog_Model_Product $prod, Mage_CatalogInventory_Model_Stock_Item $item, $sku, $qohOnly, DOMDocument $xd) {
	...
	private static function writeProductOption(DOMElement $ndOpt, Mage_Catalog_Model_Product $prod, Mage_CatalogInventory_Model_Stock_Item $item, Mage_Catalog_Model_Product_Option $option, DOMDocument $xd) {
		...
		foreach ($option->getValues() as $optId => $optValue) {
			...
			if ($hasCode) {
				self::xmlAppend("Code", $optValue->getData('sku'), $ndOptVal, $xd);
			}
...
//        Mage :: log($storeId);
//        Mage :: log("lastentity ".$lastEntityId);
//        if($lastEntityId){
            $sql = $db->select()
                ->from($ordersTable, 'entity_id')
                ->where('store_id=?', $storeId, Zend_Db::INT_TYPE)
                ->where('status="alipay_wait_seller_send_goods"')
                ->where(  "updated_at >= '$gmtTime' $entityStr" ) // "entity_id > $lastEntityId"
// ALERTBOT BEGIN => Exclude orders from sneakerhead.com domain
                ->where("coalesce(`customer_email`, '') NOT IN ('alertbot@sneakerhead.com')")
// ALERTBOT END
                ->limit($batchsize, $startnum);
...
//PCN END
		...
		if (!$order->getBillingAddress()) {
			if (self::$_debug) {
				echo "Order {$order->getData('increment_id')} was missing a billing address, skipping.";
			}
			return false;
		}
		...
		self::writeOrderAddress($ndBill,$addr, $order->getData('customer_email'), $xd);
...
//PCN BEGIN
		self::xmlAppend("Phone", preg_replace("/[^0-9\.\+\-\s\(\)]/", "", $addr->getData('telephone')), $nd, $xd);
//PCN END
	...
	private static function writeOrderItem(DOMElement $nd, Mage_Sales_Model_Order_Item $item, Mage_Sales_Model_Order $order, DOMDocument $xd) {
		...
		if (!$ok && !$isBundleItem && isset($opt['info_buyRequest']) && isset($opt['info_buyRequest']['options'])) {
			...
			if (!is_array($opt->getData('options')) || sizeof($opt->getData('options')) == 0) {
				return;
			}
	...
	private static function writeOrderOption(DOMElement $nd, $xd, $opName, $opVal, $opCode = '', $opPrice = 0) {
		...
		if ($opCode != '') {
			self::xmlAppend("OptionCode", $opCode, $ndOpt, $xd);
		}
		if ($opPrice != 0) {
			self::xmlAppend("OptionPrice", $opPrice, $ndOpt, $xd);
		}
		$nd->appendChild($ndOpt);
	}

	private static function writeOrderOptions(DOMElement $nd, $optId, $optValId, Mage_Catalog_Model_Product $prod, DOMDocument $xd) {
		$opPrice = 0;
		$selectedVal = '';
		$opName = '';
		$opCode = '';
	...
	private static function writeOrderPayment(DOMElement $ndOrd, Mage_Sales_Model_Order $order, DOMDocument $xd) {
		...
		switch ($payMeth) {
			...
			case 'usaepay':
				...
				if (!$ccName) {
					$ccName = $order->getBillingAddress()->getName();
				}
				...
				if ($transId !== false) {
					...
					if ($transId) {
						self::xmlAppend("TransID", $transId, $ndCc, $xd);
					}
				}

				if ($authCode !== false) {
					if (!$authCode) {
						$authCode = self::getCcApproval($payment);
					}
					if ($authCode) {
						self::xmlAppend("AuthCode", $authCode, $ndCc, $xd);
					}
				}
				...
				$avs = '';
				if (method_exists($payment, "getCcAvsStatus")) {
					$avs = $payment->getCcAvsStatus();
				}
				if ($avs == '') {
					$avs = $payment->getData('cc_avs_status');
				}
//PCN BEGIN
...
//PCN END
            ...
            case 'alipay_payment':
                ...
                $select = $connection->select()
                    ->from('alipay_log', array('trade_no','type'))
                    ->where('order_id=? and (type ="TRADE_SUCCESS" or  type="weixinsuccess" or `type`="success"  or `type`="TRADE_FINISHED" and 2014080851844955 <>"") ',$order->getData('increment_id'));
                $res = $connection->fetchRow($select);
                $tradeNum = null;
                $tradeStatus = null;
                if(!empty($res)){
                    $tradeNum = $res['trade_no'];
                    $tradeStatus = $res['type'];
                }
                if($tradeStatus == "TRADE_SUCCESS" ){
                    self::xmlAppend("Name", "Cnpay", $nd, $xd);
                    self::xmlAppend("Description", "网银在线 ", $nd, $xd);
                }else if($tradeStatus == "TRADE_FINISHED" ){
                    self::xmlAppend("Name", "Cnpay", $nd, $xd);
                    self::xmlAppend("Description", "Alipay ", $nd, $xd);
                    ...
                    self::xmlAppend("Name", "Cnpay", $nd, $xd);
                    self::xmlAppend("Description", "Unkown pay ", $nd, $xd);
                }

                self::xmlAppend("TradeNo",$tradeNum, $nd, $xd);
	...
	private static function getCcTransId(Varien_Object $payment) {
		...
		if (!$transId && method_exists($payment, "getRefundTransactionId")) {
			$transId = $payment->getRefundTransactionId();
		}
		if (!$transId) {
	...
	private static function getCcApproval(Varien_Object $payment) {
		$authCode = false;
		if (method_exists($payment, "getCcApproval")) {
			$authCode = $payment->getCcApproval();
		}
		if (!$authCode) {
			$authCode = $payment->getData('cc_approval');
		}
		return $authCode;
	}
...
//PCN BEGIN
        $shipDescription = $order->getData('shipping_description');
		if ($order->getData('shipping_description') == 'UPS Worldwide Expedited') {
			self::xmlAppend("Description", "International Express", $ndShip, $xd);
		} else if (stristr($shipDescription,'申通')) {
            self::xmlAppend("Description", "申通", $ndShip, $xd);
        }  else if (stristr($shipDescription,'圆通')) {
            self::xmlAppend("Description", "圆通", $ndShip, $xd);
        } else if (stristr($shipDescription,'顺丰')) {
            self::xmlAppend("Description", "顺丰", $ndShip, $xd);
        }else {
			self::xmlAppend("Description", $order->getData('shipping_description'), $ndShip, $xd);
		}
//PCN END
	...
	private static function writeOrderAdjustments(DOMElement $ndOrd, Mage_Sales_Model_Order $order, DOMDocument $xd) {
		if ((float) $order->getData('discount_amount') == 0) {
			return;
		}
...
//PCN END
	...
	private static function trackingNumberExists($trackNum, $shipments) {
		if (!is_array($shipments))
			return false;
		if (!isset($shipments['tracks']) || count($shipments['tracks']) == 0)
			return false;

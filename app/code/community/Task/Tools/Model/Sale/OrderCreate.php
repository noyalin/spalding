<?php
class Task_Tools_Model_Sale_OrderCreate extends Mage_Core_Model_Abstract
{
    private $_storeId = '21';
    private $_groupId = '1';
    ...
    public function setOrderInfo(Varien_Object $sourceOrder, Mage_Customer_Model_Customer $sourceCustomer)
    {
        ...
        $customerAddressId = $this->_sourceCustomer->getDefaultBilling();
        if ($customerAddressId){
            $address = Mage::getModel('customer/address')->load($customerAddressId);
//            echo "<pre>"; print_r($address->getData());echo "</pre>";
        }
//        var_dump( $this->_sourceCustomer->getDefaultBilling());
//You can extract/refactor this if you have more than one product, etc.
        $this->_product = Mage::getModel('catalog/product')->getCollection()
            ->addAttributeToFilter('sku', '316383-400-11')
            ->addAttributeToSelect('*')
            ->getFirstItem();
//Load full product data to product object
//        die($this->_sourceCustomer->getFirstname());
    ...
    public function create()
    {
        ...
        if (!empty($orderData)) {
            ...
            try {
                ...
                $product2 = Mage::getModel('catalog/product')->getCollection()
                    ->addAttributeToFilter('sku', '316383-400-11')
                    ->addAttributeToSelect('*')
                    ->getFirstItem();

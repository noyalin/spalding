 ...
 */
class CosmoCommerce_Alipay_Model_Wappayment extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function logTrans($trans,$type){
        $tradeNo = '';
        if(array_key_exists('trade_no',$trans)){
            $tradeNo = $trans['trade_no'];
        }
	...
	{
		return Mage::getUrl('alipay/payment/wapnotify/', array('_secure' => true));
	}
    ...
    public function getStandardCheckoutFormFields()
    {
		...
		$bank = $this->getBank();
		if($this->getConfigData('service_type')=="create_forex_trade" && $bank == "alipay"){
...
//							   'total_fee'             => sprintf('%.2f', $converted_final_price) ,
                               'rmb_fee'             => sprintf('%.2f', $converted_final_price) ,
							   'currency'      => 'USD'
							);
    ...
    public function getCallBackUrl(){
        Mage::getUrl('alipay/payment/wapnotify/', array('_secure' => true));
    }
    ...
    public function submitData(){
        ...
        $merchant_url = $this->getCallBackUrl();
        $req_data = '<direct_trade_create_req><notify_url>' . $notify_url . '</notify_url>
...
//构造要请求的参数数组，无需改动
        ...
        //建立请求
        $alipaySubmit =  Mage::getModel('alipay/wapalipaysubmit');

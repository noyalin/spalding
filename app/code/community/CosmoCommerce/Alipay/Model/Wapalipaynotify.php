...
class CosmoCommerce_Alipay_Model_Wapalipaynotify extends CosmoCommerce_Alipay_Model_Alipayabstract {
    /**
     * HTTPS形式消息验证地址
     */
...
//如果签名方式设置为“0001”时，请设置该参数
        $alipay_config['private_key_path']	= $codePath. '/key/rsa_private_key.pem';

//支付宝公钥（后缀是.pen）文件相对路径
//如果签名方式设置为“0001”时，请设置该参数
        $alipay_config['ali_public_key_path']= $codePath. '/key/alipay_public_key.pem';
...
//请保证cacert.pem文件在当前文件夹目录中
        $alipay_config['cacert']    = $codePath.'/cacert.pem';
...
//访问模式,根据自己的服务器是否支持ssl访问，若支持请选择https；若不支持请选择http
	...
	function verifyNotify($postData){
		if(empty($postData)) {//判断POST来的数组是否为空
			...
			//对notify_data解密
			$decrypt_post_para = $postData;
			if ($this->alipay_config['sign_type'] == '0001') {
				$decrypt_post_para['notify_data'] = $this->rsaDecrypt($decrypt_post_para['notify_data'], $this->alipay_config['private_key_path']);
			}
     ...
     * @return 验证结果
     */
	function verifyReturn($postData){
		if(empty($postData)) {//判断GET来的数组是否为空
			return false;
		}
		else {
			//生成签名结果
			$isSign = $this->getSignVeryfy($postData, $postData["sign"],true);
	...
	function decrypt($prestr) {
		return $this->rsaDecrypt($prestr, trim($this->alipay_config['private_key_path']));
	}
	...
	function getSignVeryfy($para_temp, $sign, $isSort) {
		...
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "MD5" :
				...
				break;
			case "RSA" :
                mage :: log(trim($this->alipay_config['ali_public_key_path']));
